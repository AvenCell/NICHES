#### build the base SCC matrix with all cells connected
#### TODO 1: change the hierarchy of the scc_base functions (probably abandon the expmat format)
#### TODO 2: break down the scc_base functions into different functions

#' SCC base function
#'
#' Construct the fully connected cell-pair ligand-receptor expression profiles
#'
...
####TODO: the cell_types_vec is not necessary
#' scc_base default
#' @param cell_types_vec character vector, default: NULL
#' cell type annotations for each cell in the expressoin matrix, used for scc_base.Default
#' @param metadata_lst character list, optional, default: NULL
#' what metadata to aggregate, used for scc_base.Default
#' @method scc_base default
#'
#' @export
#'

scc_base.default <- function(exp_mat,species="human",cell_types_vec=NULL,metadata_lst=NULL,downsampling = TRUE,sample_size_n=1000,autocrine=TRUE,n_threads=8){
...
#' The assay to use from the input Seurat Object, NULL leads to the default assay, used for scc_base.Seurat
#' @param metadata character vector, default: NULL
#' The metadata to aggregate: e.g. cell_types, positions, used for scc_base.Seurat
#' @method scc_base Seurat
#' @export
####metadata: a vector of metadata to aggregate to the output (a vector of names)
####if cell_types want to be aggregated, it must be present inside metadata
####for now, it has to be called as cell_types inside
scc_base.Seurat <- function(
  seu_obj,
  assay=NULL,
  metadata,
  species="human",
  ...
  result <- scc_base(exp_mat=Seurat::GetAssayData(object = seu_obj, slot = 'data', assay = assay),
                 species=species,
